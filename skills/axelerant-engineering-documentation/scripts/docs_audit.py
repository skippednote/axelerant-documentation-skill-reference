...
FLUFF_TERMS = (Path(__file__).resolve().parent.parent
               / "references" / "fluff-terms.txt")


def load_fluff():
    """One source for the deny-list, shared with the Vale style.

    Two hand-maintained copies drifted by three terms inside a day, which is
    the same failure the contract warns about for documentation.
    """
    if not FLUFF_TERMS.exists():
        return None, []
    lines = [ln.strip() for ln in FLUFF_TERMS.read_text().splitlines()]
    terms = [ln for ln in lines if ln and not ln.startswith("#")]
    joined = "|".join("(?:%s)" % t for t in terms)
    return re.compile(r"\b(?:%s)" % joined, re.I), terms


FLUFF, FLUFF_LIST = load_fluff()
BANNED_PATHS = [
...
def check_relative_links(root):
    """Resolve relative markdown links without the network.

    CI runs a full link checker, but that needs a runner. This catches the
    common case, a link to a file that was renamed or never written, at the
    moment the agent finishes writing.
    """
    for f in sorted(root.rglob("*.md")):
        rel = f.relative_to(root)
        if any(p in (".git", "node_modules", "vendor", ".docs-standard")
               for p in rel.parts):
            continue
        if ignored(rel):
            continue
        for m in MDLINK.finditer(f.read_text(errors="ignore")):
            href = m.group(1).split("#")[0]
            if not href or href.startswith(("http://", "https://", "mailto:", "<")):
                continue
            if not (f.parent / href).exists():
                add("BLOCK", str(rel), f"link to {href}, which does not exist")
...
def emit_vale():
    """Print the Vale style generated from fluff-terms.txt."""
    print("# Generated by docs_audit.py --emit-vale. Do not edit by hand;")
    print("# the source is references/fluff-terms.txt.")
    print("extends: existence")
    placeholder = "'%s'"          # Vale substitutes the matched token here
    print('message: "Fluff: ' + placeholder + ' carries no information. '
          'Cut it or replace it with the specific fact."')
    print("level: error")
